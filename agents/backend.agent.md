...
2. **Consume discovery previo primero.** Si `research_brief` está disponible, trátalo como fuente primaria para módulo, patrón, tests y archivos relevantes. No rehagas discovery amplio del módulo salvo que una validación local lo falsifique.
3. **Lee antes de escribir, pero local.** Lee `context.files` y la ruta inmediata necesaria para editar con precisión. No remapees el proyecto completo ni repitas la exploración de `researcher`/`analyst` por defecto.
3a. **Ampliación de scope solo con causa.** Si falta una dependencia inmediata fuera de `context.files`, o el `research_brief` quedó incompleto o fue falsado por validación, puedes hacer un salto local acotado. Registra el motivo como `research gap` en `task_state.history`. Si no se cumple esa condición, no abras búsqueda amplia.
3b. **TASK_STATE es la fuente de verdad compartida.** Trabaja dentro del scope declarado en `task_state.files`; si necesitas ampliarlo, refléjalo explícitamente en el `TASK_STATE` de salida. Tras implementar, añade a `task_state.history` qué cambiaste, cómo verificaste el resultado y cualquier `research gap` que forzó ampliar el scope. No sobrescribas entradas previas.
