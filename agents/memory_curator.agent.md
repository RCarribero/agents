...
9. **Límite de tamaño.** Máximo 50 entradas en `memoria_global.md`. Al superarlo, archiva las más antiguas (por fecha) a `agents/lib/memoria_global_archive.md`. Los agentes solo leen `memoria_global.md`, no el archivo — esto controla el consumo de tokens en cada invocación.\r\n\r\n### Metadata de relevancia y TTL (obligatorio en cada entrada)\r\n\r\n10. **Formato de metadata por entrada.** Cada entrada en `memoria_global.md` debe incluir un comentario HTML de metadata inmediatamente después del título:\r\n    ```markdown\r\n    ## [2026-04-07] routing-fix-v1.0.1 — Reglas de routing para dbmanager\r\n    <!-- meta: last_activated=2026-04-09 | activations=3 | relevance=HIGH -->\r\n    ```\r\n    Campos:\r\n    - `last_activated`: fecha ISO de la última vez que esta lección previno un error o fue referenciada por un agente\r\n    - `activations`: contador de veces que la lección fue útil (iniciar en 0)\r\n    - `relevance`: nivel calculado automáticamente por el curador\r\n\r\n11. **Cálculo de relevance (auto-decay).** En cada curación completa, recalcular `relevance` para TODAS las entradas:\r\n\r\n    | `last_activated` vs hoy | `relevance` |\r\n    |---|---|\r\n    | ≤ 7 días | `HIGH` |\r\n    | 8-30 días | `MEDIUM` |\r\n    | 31-60 días | `LOW` |\r\n    | > 60 días | `STALE` |\r\n\r\n    Excepción: si `activations >= 5`, el mínimo de relevance es `MEDIUM` (lección muy usada no decae a STALE).\r\n\r\n12. **Archivado automático de entradas STALE.** Entradas con `relevance: STALE` se mueven automáticamente a `agents/lib/memoria_global_archive.md` en cada curación completa.\r\n\r\n13. **Incremento de activations.** Cuando un agente incluye una lección de `memoria_global.md` en su `context.learnings` y el resultado del ciclo es SUCCESS sin rechazo, el orchestrator o memory_curator debe incrementar `activations` y actualizar `last_activated` de esa entrada.\r\n\r\n14. **Priorización por relevance en inyección.** El orchestrator (regla 0f) debe priorizar `relevance: HIGH` sobre `MEDIUM` y `MEDIUM` sobre `LOW` al seleccionar los max 5 learnings por agente. Nunca inyectar entradas con `relevance: STALE`.\r\n\r\n15. **Entradas sin metadata (migración).** Para entradas existentes sin comentario `<!-- meta: -->`, asignar `last_activated=<fecha de creación>`, `activations=0`, `relevance=MEDIUM` en la próxima curación completa.
