...
- Usa `TASK_STATE` como estado compartido del swarm. Mantén al menos estos campos: `task_id`, `goal`, `plan`, `current_step`, `files`, `risk_level`, `attempts`, `history`. Se permiten extensiones del proyecto (`constraints`, `risks`, `artifacts`, etc.), pero nunca sustituyen ese núcleo.
